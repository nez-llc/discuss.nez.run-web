    ...
    <div css={css`display: flex;`}>
      ...
      <div css={css`width:70%; display: grid; gap: 10px;`}>
        <div css={css`width: 100%; padding: 0px 10px; height: 120px;`}>
          <h4 css={css`height: 80px; display: flex; align-items: self-end;`}>{agenda.title}</h4>
  ...
  <>
    ...
    <div css={css`width: 100%; display: flex; flex-direction: column; gap: 10px;`}>
      ...
      <div css={css`font-size: small; dl {display: flex;} dd {margin-right: 16px;}`}>
        <dl>
          <dt>추천</dt>
          <dd>{agenda.updown?.total}</dd>
          <dt>투표</dt>
          <dd>{sum(Object.values(agenda.vote_count))}</dd>
          <dt>댓글</dt>
          <dd>{agenda.comment_count}</dd>
          <dt css={css`margin-left: auto;`}>{detailDate(agenda.created_time)}</dt>
        </dl>
      </div>
      <VoteBar voteCount={agenda.vote_count} />
    </div>
...
const ListView3 = ({ agenda }) => (
  <>
    <div css={css`display: flex; gap: 5px;`}>
      <div css={css`width:70%; display: flex; flex-direction: column; gap: 10px;`}>
        <h4 css={css`height: 70px`}>{agenda.title}</h4>
        <ul css={css`display: flex; gap: 3px;`}>
          <li css={css`width:20%; font-size: 11px; text-align: center;`}>{detailDate(agenda.created_time)}</li>
          <li css={css`width: 60%;`}>
            <VoteBar voteCount={agenda.vote_count} />
          </li>
...
const AgendaPreview = ({ agenda, view }) => {
  switch (view) {
    case 'L01':
      return (
        <Wrapper>
    ...
    case 'S01':
      return (
        <div css={css`text-decoration: none;`}>
          <div css={css`background-color: #bdbdbd; background-image: url("https://cdn.pixabay.com/photo/2019/07/23/13/51/shepherd-dog-4357790_1280.jpg"); background-repeat: no-repeat; background-size: cover;`}>
            <div
              css={css`
                  height: 250px;
              ...
              <h2 css={css`color: #fff; min-height: 80px; display: flex; align-items: self-end;`}>{agenda.title}</h2>
              <VoteBar voteCount={agenda.vote_count} />
        ...
        <div css={css`text-decoration: none;`}>
          <div css={css`background-color: #bdbdbd; background-image: url("https://cdn.pixabay.com/photo/2019/07/23/13/51/shepherd-dog-4357790_1280.jpg"); background-repeat: no-repeat; background-size: cover;`}>
            <div
              ...
              <h2 css={css`color: #fff; padding: 0px 20px; min-height: 120px; display: flex; align-items: self-end;`}>{agenda.title}</h2>
              <div css={css`display: flex;
                  align-items: center;
                  padding: 8px 12px;
                  gap: 8px;
